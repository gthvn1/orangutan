...
module Ast = struct
  ...
  (** [s] represents all statements in Monkey. Currently, the only statement
  ...
  let token_literal node : string =
    match node with
    | Statement _ -> failwith "TODO: implement token_literal for statement"
    | Expression _ -> failwith "TODO: implement token_literal for expression"
end
