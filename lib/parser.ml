...
let parse_let_statement (parse : t) : (A.Statement.t, string) result =
...
let parse_program (parse : t) : A.Program.t =
  ...
  let rec parse_stmts (parse : t) (prog : A.Program.t) : A.Program.t =
    if fst parse = T.EOF then
      ...
      | Ok s ->
          parse_stmts (next_token parse)
            { stmts = s :: prog.stmts; errors = prog.errors }
  in
